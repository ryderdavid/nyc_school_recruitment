...
sheet_19 <- read_xlsx("data/historical_recruitment_data_0319.xlsx")
sheet_18 <- read_xlsx("data/historical_recruitment_data.xlsx", sheet = 2)
sheet_17 <- read_xlsx("data/historical_recruitment_data.xlsx", sheet = 3)
...
# tmap_arrange(regs_2017, regs_2018, nrow = 1)
...
rec_rates_2017 <- tracts_by_recruitment_performance %>% filter(Year == 2017)
tm_shape(rec_rates_2017) + tm_borders(alpha = 0.2) + 
  tm_fill(col = "Recruitment Rate")

rec_rates_2018 <- tracts_by_recruitment_performance %>% filter(Year == 2018)
tm_shape(rec_rates_2018) + tm_borders(alpha = 0.2) + 
  tm_fill(col = "Recruitment Rate",
          palette = "BuGn")

# rec_rates_2019 <- tracts_by_recruitment_performance %>% filter(Year == 2018)
# tm_shape(rec_rates_2018) + tm_borders(alpha = 0.2) + 
#   tm_fill(col = "Recruitment Rate",
#           style = "jenks")
...
Focusing on Manhattan and the Bronx, this report analyzed NYC's Department of Education enrollment statistics in `r length(unique(districts_demo_snapshot_1718$school_dist))` districts, to determine best advertisement placements for reaching English Language Learner students.
...
```{r, housing_dataset, include=FALSE}

nycha_stats <- read_csv("https://data.cityofnewyork.us/api/views/evjd-dqpz/rows.csv?accessType=DOWNLOAD")


nycha_sf <- st_read("https://data.cityofnewyork.us/resource/5j2e-zhmb.geojson",
                    stringsAsFactors = F) %>% 
  st_transform(crs = wgs84_crs)


nycha_developments <- inner_join(nycha_sf, nycha_stats, by = c("tds_num" = "TDS#"))
...
tm_tiles(providers$CartoDB.PositronNoLabels) + 
  tm_shape(nyc_sds, point.per = "feature") +
    tm_borders(alpha = 0.7) +
    tm_text(text = "school_dist") +
  tm_shape(nycha_developments) + 
    tm_borders(alpha = 0.2) +
    tm_fill(col = "POPULATION PUBLIC HOUSING",
          id="tractname",
          style = "jenks",
          title = "Pub. Housing Pop",
          popup.vars = c("TDS#"="tds_num",
                         "Pub. Housing Population"="POPULATION PUBLIC HOUSING",
                         "Development"="DEVELOPMENT")) +
    tm_view(set.view = c(-73.86652, 40.85263, 11),
          set.zoom.limits = c(12,14)) + 
  tm_layout(outer.margins = 20)
# 
# 
# 
# nycha_addresses_text <- pdftools::pdf_text("https://www1.nyc.gov/assets/nycha/downloads/pdf/Address-Guide-01142019.pdf")
# 
# text <- readtext::readtext("https://www1.nyc.gov/assets/nycha/downloads/pdf/Address-Guide-01142019.pdf")
# 
# testtt <- extract_tables("https://www1.nyc.gov/assets/nycha/downloads/pdf/Address-Guide-01142019.pdf",
#                          output = "data.frame")
# 
# testtt
