...
plot_recruitment_rate_map <- function (year = seq(1950,2050), palette = "RdYlBu", data = recruitment_data_clean) {
  require(lubridate)
  y <- year
  p <- palette
  d <- data
  ...
  d %>% count(sub_yr, sub_m) -> plt
  
  ggplot(plt) + geom_smooth(aes(x = interaction(sub_yr, sub_m), y = n), group = 1)
...
d <- d %>% mutate(submission_date = lubridate::mdy_hms(submission_date))

d %>% mutate(submission_date = floor_date(submission_date, unit = "month"))

d <- d %>% mutate(sub_yr = lubridate::year(submission_date),
                  sub_m = lubridate::month(submission_date))
