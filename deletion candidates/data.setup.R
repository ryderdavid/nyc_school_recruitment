...
points_over_polys <- function (points = NULL, polys = NULL, year = NULL, palette = "YlOrBr") {
  ...
  # pols = polys
  # yr = year
  # pts <- points %>% filter(year %in% yr) %>% select(year)
  # p <- palette
    

  yr <-  2017
  pts <-  application_points %>% filter(year %in% yr) %>% select(year)
  pols <- nyc_sds
  p <- "Reds"
  
  pols <- nyc_tracts
  
  #  identify which points intersect with which polygons, flatten those to a
  #  count by applying length() to each list item, and bind the result of that
  #  as a column to the original polygon shapefile
  merge <- 
    pols %>% st_intersects(., pts) %>% 
    sapply(length) %>% 
    bind_cols(pols, n=.) %>% 
    filter(n > 0)
  
  # tmap_mode("view")
  # tm_shape(merge, point.per = "feature") +
  #   tm_borders(alpha = 0.2) + 
  #   tm_fill(col = "n", palette = p) -> tx
...
apps.2017.sd      <- points_over_polys(application_points, nyc_sds, 2017)
apps.2017.tracts  <- points_over_polys(application_points, nyc_tracts, 2017)

apps.2018.sd      <- points_over_polys(application_points, nyc_sds, 2018)
apps.2018.tracts  <- points_over_polys(application_points, nyc_tracts, 2018)

apps.2019.sd      <- points_over_polys(application_points, nyc_sds, 2019)
apps.2019.tracts  <- points_over_polys(application_points, nyc_tracts, 2019)
