...
library(widgetframe) # for better plotting of leaflet maps
```
...
Most of the applications are being received from the southern Bronx, with some from the northern part of Manhattan. The largest number of applications received from a single tract was `r applications_by_tract_yearly_totals$n %>% max()`.



```{r applications_plot, out.width='100%'}
...
tmap_arrange(a_2017, a_2018, a_2019, nrow = 1, asp = 0)
...
Similarly, most of the registrations completed are being received from the southern Bronx, with some from the northern part of Manhattan. The largest number of registrations completed from a single tract was `r registrations_by_tract_yearly_totals$n %>% max()`.
...
tmap_arrange(a_2017, a_2018, a_2019, asp = 1, nrow = 1)
