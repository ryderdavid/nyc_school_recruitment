...
```{r setup, results='hide', cache=TRUE}
...
trct <- 
  tigris::tracts(state = "NY", 
                 county = c('Bronx',
                            ...
                            'Richmond')) %>% 
  as("sf") %>%  # cast to sf
  st_transform(crs=wgs84_crs)  # apply common CRS
...
recruitment_data %<>% 
  ...
  mutate(year        = as.numeric(year),
         ...
         zip         = as.factor(as.integer(GEOID10)),
...
```{r results='hide'}
points_over_polys <- function (points = NULL, polys = NULL, year = NULL) {
...
regs.2017.sd      <- points_over_polys(registration_points, nyc_sds, 2017)
regs.2017.tracts  <- points_over_polys(registration_points, nyc_tracts, 2017)

regs.2018.sd      <- points_over_polys(registration_points, nyc_sds, 2018)
regs.2018.tracts  <- points_over_polys(registration_points, nyc_tracts, 2018)
...
a2017 <- 
  ...
  tm_shape(apps.2017.tracts) + 
  tm_borders(alpha = 0.5) + 
  tm_fill(col = "n",
          title = "Applications 2017",
          id="NAMELSAD",
...
a2018 <- 
  tm_tiles(leaflet::providers$CartoDB.PositronNoLabels) + 
  tm_shape(apps.2018.tracts) + 
  tm_borders(alpha = 0.5) + 
  tm_fill(col = "n",
          id="NAMELSAD",
          palette = "Greens",
          title = "Applications 2018",
          popup.vars = c("Applications"="n")) + 
  tm_view(set.view = c(-73.86652, 40.85263, 11))

a2019 <- 
  tm_tiles(leaflet::providers$CartoDB.PositronNoLabels) + 
  tm_shape(apps.2019.tracts) + 
  tm_borders(alpha = 0.5) + 
  tm_fill(col = "n",
          id="NAMELSAD",
          palette = "Reds",
          title = "Applications 2019",
          popup.vars = c("Applications"="n")) + 
  tm_view(set.view = c(-73.86652, 40.85263, 11))
...
tmap_mode("view")
tmap_arrange(a2017, a2018, a2019, ncol = 3, sync = T, asp = 1)

```
\  
Above is the distribution of applications received, aggregated by census tract. 
\  
...
r2017 <- 
  tm_tiles(leaflet::providers$CartoDB.PositronNoLabels) + 
  tm_shape(regs.2017.tracts) + 
  tm_borders(alpha = 0.5) + 
  tm_fill(col = "n",
          title = "Registrations 2017",
          popup.vars = c("Applications"="n"),
          legend.format = ) + 
  tm_view(set.view = c(-73.86652, 40.85263, 11))
...
r2018 <- 
  tm_tiles(leaflet::providers$CartoDB.PositronNoLabels) + 
  tm_shape(regs.2018.tracts) + 
  tm_borders(alpha = 0.5) + 
  tm_fill(col = "n",
          palette = "Greens",
          title = "Registrations 2018",
          popup.vars = c("Applications"="n")) + 
  tm_view(set.view = c(-73.86652, 40.85263, 11))

tmap_mode("view")
tmap_arrange(r2017, r2018, ncol = 2, sync = T)

```
\  
Above is the distribution of students registered, aggregated by census tract. 
\  

```{r}

xx <- apps.2017.sd %>% filter(.$n > median(apps.2017.sd$n))

xx


ggplot(apps.2017.sd) + geom_bar(aes(x = school_dist, y = n), stat = "identity")
```
...
### Application to Registration Performance
...
tmap_mode("view")
  ...
  tm_view(set.view=c(st_centroid(bronx_sf)[[1]] %>% as.numeric(), 12),
